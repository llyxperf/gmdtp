...
pub type Seckey = BigUint;
    ...
    pub fn getSm2key()-> (SigCtx, Option<Point>,  Option<BigUint>) {
        ...
        if n.unwrap()==0{
            ...
            f.write_all(&sk_raw[..]);
            return (ctx,Some(pk),Some(sk));

        }
        else{
          
            let sknew = ctx.load_seckey(&buffer[..]).unwrap();

            let pk = ctx.pk_from_sk(&sknew);
            
            return (ctx,Some(pk),Some(sknew));
        }
